...
int judge_point(int x0, int y0, int x1, int y1, int x, int y) {
  dx = x1 - x0;
  dy = y1 - y0;
  j  = -(x-x0)*dy + (y-y0)*dx
  if (j == 0)
...
void DrawRend::rasterize_triangle( float x0, float y0,
  ...
  //  samplebuffer[x2][y2].fill_pixel(color);
      ...
      for (double i = 0; i < total_height; i++) {
        ...
        double beta = 0;
        if (segment_height != 0)
          beta  = (i-(second_half ? y1-y0 : 0))/segment_height; // be careful: with above conditions no division by zero here
        double xa = x0 + (x2 - x0)*alpha;
        double xb = second_half ? x1 + (x2-x1)*beta : x0 + (x1-x0)*beta;

        if (xa>xb) std::swap(xa, xb);
        for (float j = xa; j <= xb; ++j)
          DrawRend::rasterize_point(j+0.5, y0+i+0.5, color);

      }
      DrawRend::rasterize_line(x0, y0, x1, y1, color);
      DrawRend::rasterize_line(x1, y1, x2, y2, color);
      DrawRend::rasterize_line(x0, y0, x2, y2, color);

      double ymax = 0;
      if (y2>y1) ymax = y2>y0?y2:y0;
      else ymax = y1>y0?y1:y0;
      double ymin = 99999;
      if (y2>y1) ymin = y1>y0?y0:y1;
      else ymin = y2>y0?y0:y2;
      double xmax = 0;
      if (x2>x1) xmax = x2>x0?x2:x0;
      else xmax = x1>x0?x1:x0;
      double xmin = 99999;
      if (x2>x1) xmin = x1>x0?x0:x1;
      else xmin = x2>x0?x0:x2;

      for (double i = xmin; i < xmax; ++i) {
        for (double j = ymin; j < ymax; ++j) {
          int ja = judge_point(x0, y0, x1, y1, i+0.5, j+0.5);
          int jb = judge_point(x0, y0, x2, y2, i+0.5, j+0.5);
          int jc = judge_point(x1, y1, x2, y2, i+0.5, j+0.5);
          if (ja >= 0 && jb >= 0 && jc >= 0)
            DrawRend::rasterize_point(i, j, color);
        }
      }
